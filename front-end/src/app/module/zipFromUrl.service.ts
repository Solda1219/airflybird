...
export class ZipFromUrlService {
  ...
  async createSnapImageZip(item, zipName: string) {  
    const zip = new JSZip();  
    const name = zipName + '.zip';  
    // for image
    for (let counter = 0; counter < item.length; counter++) {  
      const element = item[counter];  
      for(let i = 0; i < element.child_item.length; i++){
        const child = element.child_item[i];
        const fileData: any = await this.getFile(child.image_url);
        const b: any = new Blob([fileData], { type: '' + fileData.type + '' });  
        zip.file(''+element.group_name+'/'+element.group_name+'-'+(i+1)+child.image_url.substring(child.image_url.lastIndexOf('.')), b); 
      }
    }  
    zip.generateAsync({ type: 'blob' }).then((content) => {  
      if (content) {  
