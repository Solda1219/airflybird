...
@Component({
  selector: 'app-eq-camera-table',
  templateUrl: './eq-camera-table.component.html',
  styleUrls: ['./eq-camera-table.component.scss']
})
export class EqCameraTableComponent implements OnInit {
  @Input('item') item = [];
  //for mat-table
  tbHeader = ['Camera ID', 'Camera Type', 'IMEI', 'car Number', 'Install Date', 'Expire Date','GPU'];
  tbCol = ['camera_id', 'camera_type', 'device_imei', 'car_number', 'install_date', 'end','gpu_name'];
  displayedColumns: string[] = ['No', 'camera_id', 'camera_type', 'device_imei', 'car_number', 'install_date', 'end','gpu_name', 'Action'];
  specialCol = [];
  ...
  resetTerm(){
     this.resetItems.emit(this.modalItem);
  }
  delList(){
