...
@Component({
  selector: 'app-eq-gpu-table',
  templateUrl: './eq-gpu-table.component.html',
  styleUrls: ['./eq-gpu-table.component.scss']
})
export class EqGpuTableComponent implements OnInit {
  @Input('item') item = [];
  //for mat-table
  tbHeader = ['Server Name', 'Location', 'Status', 'Owner', 'Created Time'];
  tbCol = ['gpu_address', 'gpu_location', 'status', 'owner', 'created_at'];
  // tbHeader = ['Server Name', 'Location', 'Status', 'Owner', 'Created Time', 'Detecting Video Name','Detecting Start Time'];
  // tbCol = ['gpu_address', 'gpu_location', 'status', 'owner', 'created_at', 'video_name','detect_start'];
  displayedColumns: string[] = ['No', 'gpu_address', 'gpu_location', 'status', 'owner', 'created_at', 'Action'];
  specialCol = ['status','video_name','detect_start'];
  ...
  @ViewChild('cameraListModal') cameraListModal:ModalDirective;
  cameraList = [];
  constructor(
  ...
  async showCameraModal(row){
    this.modalItem = JSON.parse(JSON.stringify(row))
    try {
      const res = await this.userService.postRequest('_api/equipment/gpu/cameras',this.modalItem).toPromise();
      this.cameraList = res['result'];
      this.cameraListModal.show()
    } catch (err) {
