...
@Component({
  selector: 'app-report-alert-detail',
  templateUrl: './report-alert-detail.component.html',
  styleUrls: ['./report-alert-detail.component.scss']
})
export class ReportAlertDetailComponent implements OnInit {
  loading = false;
  formGroup: FormGroup;
  agents = [];
  item = [];
  ...
  async getAlertType(){
    try{
      const res = await this.userService.postRequest('_api/report/alert/getType').toPromise();
  ...
  async search(){
    ...
    this.loading = true;
    try{
      const res = await this.userService.postRequest('_api/report/alertDetail/get',data).toPromise()
  ...
  exportToExcel(){
    ...
    const title = 'Alert details';
    const subtitle = ['date',''+this.formGroup.value['startdate']+'/'+this.formGroup.value['enddate']];
    this.excel.exportAsExcelFile('AlertDetails'+'_'+new Date().getTime(),this.tbHeader,tbData,title,subtitle);
  }
  dateDefine(){
