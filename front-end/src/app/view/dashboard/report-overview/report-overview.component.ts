...
export class ReportOverviewComponent implements OnInit {
  ...
  async search(){
    ...
    this.loading = true;
    try{
      const res = await this.userService.postRequest('_api/report/overveiw/get',data).toPromise()
  ...
  exportToExcel(){
    ...
    const title = 'Report of overview';
    const subtitle = ['date',''+this.formGroup.value['startdate']+'/'+this.formGroup.value['enddate']];
    this.excel.exportAsExcelFile('ReportOverview'+'_'+new Date().getTime(),this.tbHeader,tbData,title,subtitle);
  }
  dateDefine(){
