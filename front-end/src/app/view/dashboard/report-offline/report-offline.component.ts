...
export class ReportOfflineComponent implements OnInit {
  loading = false;
  formGroup: FormGroup;
  agents = [];
  item = [];
  ...
  async search(){
    ...
    this.loading = true;
    try{
      const res = await this.userService.postRequest('_api/report/offline/get',data).toPromise()
  ...
  exportToExcel(){
    ...
    const title = 'Report of offline';
    const subtitle = ['date',''+this.formGroup.value['startdate']+'/'+this.formGroup.value['enddate']];
    this.excel.exportAsExcelFile('Reportoffline'+'_'+new Date().getTime(),this.tbHeader,tbData,title,subtitle);
  }
  dateDefine(){
