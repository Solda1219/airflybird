...
@Component({
  selector: 'app-report-overspeed',
  templateUrl: './report-overspeed.component.html',
  styleUrls: ['./report-overspeed.component.scss']
})
export class ReportOverspeedComponent implements OnInit {
  loading = false;
  formGroup: FormGroup;
  agents = [];
  item = [];
  selected_items= [];
  tbHeader = ['Device Name', 'IMEI', 'Model','Status','StartTime','Endtime', 'Address',"Duration",'Location started','Location ended'];
  tbCol = ['device_name', 'imei', 'model','status','start_time','end_time', 'start_address',"duration",'start_address','end_address'];
  constructor(
  ...
  async search(){
    ...
    this.loading = true;
    try{
      const res = await this.userService.postRequest('_api/report/overspeed/get',data).toPromise()
  ...
  exportToExcel(){
    ...
    const title = 'Report of overspeed';
    const subtitle = ['date',''+this.formGroup.value['startdate']+'/'+this.formGroup.value['enddate']];
    this.excel.exportAsExcelFile('ReportOverspeed'+'_'+new Date().getTime(),this.tbHeader,tbData,title,subtitle);
  }
  dateDefine(){
