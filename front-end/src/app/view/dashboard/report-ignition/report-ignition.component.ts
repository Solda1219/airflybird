...
@Component({
  selector: 'app-report-ignition',
  templateUrl: './report-ignition.component.html',
  styleUrls: ['./report-ignition.component.scss']
})
export class ReportIgnitionComponent implements OnInit {
  loading = false;
  formGroup: FormGroup;
  agents = [];
  item = [];
  ...
  async search(){
    ...
    this.loading = true;
    try{
      const res = await this.userService.postRequest('_api/report/ignition/get',data).toPromise()
  ...
  exportToExcel(){
    ...
    const title = 'Report of ignition';
    const subtitle = ['date',''+this.formGroup.value['startdate']+'/'+this.formGroup.value['enddate']];
    this.excel.exportAsExcelFile('ReportIginition'+'_'+new Date().getTime(),this.tbHeader,tbData,title,subtitle);
  }
  dateDefine(){
