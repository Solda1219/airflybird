...
export class ReportAlertComponent implements OnInit {
  ...
  async search(){
    ...
    this.loading = true;
    try{
      const res = await this.userService.postRequest('_api/report/alert/get',data).toPromise()
  ...
  exportToExcel(){
    ...
    const title = 'Report of alert';
    const subtitle = ['date',''+this.formGroup.value['startdate']+'/'+this.formGroup.value['enddate']];
    this.excel.exportAsExcelFile('ReportAlert'+'_'+new Date().getTime(),this.tbHeader,tbData,title,subtitle);
  }
  dateDefine(){
